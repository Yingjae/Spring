...
public interface BoardMapper {
	...
	// 글 삭제는 DELETE 구문으로 하는데, 비 SELECT 구문이므로 리턴 자료를 void로 적습니다.
	// 하나의 글만 삭제할 예정이므로, 삭제할 글 정보를 호출시 같이 입력하게 합니다.
	public void delete(long bno);
	
	// 글 수정은 UPDATE 구문으로 합니다. 메서드를 선언해주세요.
	// 전달 변수가 title, content, bno 이므로, 단일 자료가 아닌 묶음으로 전달합니다.
	public void update(BoardVO vo);
	
	// vo 안쓰고 데이터 전달하기
	// 2개 이상의 파라미터를 따로따로 전달할 때는 각 파라미터 왼쪽에 @Param을 붙여줍니다.
